...
function verifyPersonaAssertion(assertion, audience, callback) {
    ...
    */
    console.log({
        assertion: assertion,
        audience: settings.DEBUG ? audience : settings.ORIGIN
    });
...
function _userID(email) {
    return crypto.createHash('sha1').update(email + settings.SECRET).digest('hex');
}

function _hmac(key, value) {
    return crypto.createHmac('sha512', key + settings.SECRET).update(value).digest('hex');
}
